...
Can you come up with other examples where our 'sample' is not just a random subset of some population? For starters, have a think about the weather, the stock market, and the clothes you will choose to wear tomorrow.
...
popn_props <- poll_props <- intend_truman <-
  array(dim = c(3, 2), dimnames = list(demographic = 1:3, has_phone = 0:1))

popn_props[] <- outer(c(0.5, 0.3, 0.2), c(0.6, 0.4))
intend_truman[] <- c(0.4, 0.5, 0.6, 0.2, 0.4, 0.5)
vote_truman <- intend_truman + 0.1
```
...
And this shows the probability of voting for Harry Truman within each of these groups, at some point shortly before the election actually took place.
...
vote_truman <- intend_truman - 0.2 # modify this line to give Truman a late 10% boost
...
sum(poll_props * vote_truman) # polling results
sum(popn_props * vote_truman) # actual vote
```

```{r phone-solution}
poll_props[] <- c(0, 0, 0, 1/3, 1/3, 1/3) # representative poll of phone users
poll_props[] <- c(0.6, 0, 0, 0.4, 0, 0) # representative poll of demographic 1
```
...
Discuss with the person next to you similarities and differences between these last two pictures.
...
Remarkably, we have arrived at an estimate of the precision of our estimated mean, *using only the single sample we have available to us*.
...
```
Discuss with a different person the meaning of these pictures. Do any of them alarm you?
