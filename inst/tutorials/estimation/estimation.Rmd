...
TODO: replace PollEverywhere with Mentimeter here.
...
TODO: Mentimeter explanation of why I didn't turn up!
...
TODO: Mentimeter to ask people what cutoff they would choose
...
NB: it is impossible to specify ignorance! Uniform distribution is not such a statement -- rather, it is the strong statement that all models are equally likely.

### One parameter

Because the choice of model now has a probability distribution, we can form a so-called credible interval (or set) by grabbing whatever percentage of this we want.

For example, we might believe *a priori* that the model is `P2` or `P3` with probability 0.9, but *a posteriori* have updated the probability of this set to 0.8. The set {`P2`, `P3`} would be a 80% posterior credible interval. But note that, in this case, all of that 80% is in fact concentrated on `P2`!
...
> Despite the many attractions of Bayesian inference, this is still not universally accepted, basically because of the important and not-always-obvious role of the prior.
